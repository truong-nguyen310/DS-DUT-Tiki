...
def parser_product(json):
    ...
    # Kiểm tra và lấy giá trị 'quantity_sold'
    quantity_sold_data = json.get('quantity_sold')
    if quantity_sold_data:
        d['quantity_sold'] = quantity_sold_data.get('value')
    else:
        d['quantity_sold'] = None
        
    d['inventory_status'] = json.get('inventory_status')
    
    # Kiểm tra và lấy giá trị 'stock_item'
    stock_item_data = json.get('stock_item')
    if stock_item_data:
        d['stock_item_qty'] = stock_item_data.get('qty')
        d['stock_item_max_sale_qty'] = stock_item_data.get('max_sale_qty')
    else:
    ...
    # Kiểm tra và lấy giá trị 'brand'
    brand_data = json.get('brand')
    if brand_data:
        d['brand_id'] = brand_data.get('id')
        d['brand_name'] = brand_data.get('name')
    else:
        d['brand_id'] = None
        d['brand_name'] = None
    ...
    return d
# lấy smartphone
df_id = pd.read_csv('./raw_data/mobile_id_ncds.csv')
...
for pid in tqdm(p_ids, total=len(p_ids)):
    try:
        ...
        response.raise_for_status()  # Nếu có lỗi trong request thì ném exception
        if response.status_code == 200:
            print('Crawl data {} success !!!'.format(pid))
            result.append(parser_product(response.json()))
    except RequestException as e:
        print(f"Error fetching data for product ID {pid}: {e}")

df_product = pd.DataFrame(result)
df_product.to_csv('./raw_data/mobile_data_ncds.csv', index=False, encoding='utf-8-sig')

#lấy laptop
df_id = pd.read_csv('./raw_data/laptop_id_ncds.csv')
p_ids = df_id.id.to_list()
print(p_ids)
result = []
...
df_product = pd.DataFrame(result)
df_product.to_csv('./raw_data/laptop_data_ncds.csv', index=False, encoding='utf-8-sig')